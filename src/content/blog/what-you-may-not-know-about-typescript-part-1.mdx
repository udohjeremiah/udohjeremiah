...
`"strict": true{:ts}` in `tsconfig.json`, which toggles them all on simultaneously, but you can opt out of them
individually. A new codebase should always turn these strictness checks on.
...
either using compiler flags or in `tsconfig.json`, to allow emitting JavaScript output despite errors, `a{:ts}` will
have the value of `Infinity{:ts}` at runtime, even when TypeScript detects an error.
