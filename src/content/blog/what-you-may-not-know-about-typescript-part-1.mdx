...
## In TypeScript, you can use a `tsconfig.json{:.constant.other.color}` file to specify the root files and the compiler options required to compile the project.

The presence of a `tsconfig.json{:.constant.other.color}` file in a directory indicates that the directory is the root
of a TypeScript project.

The `tsconfig.json{:.constant.other.color}` file specifies the root files and the compiler options required to compile
the project. JavaScript projects can use a `jsconfig.json{:.constant.other.color}` file instead, which acts almost the
same but has some JavaScript-related compiler flags enabled by default.
...
`"strict": true{:ts}` in `tsconfig.json{:.constant.other.color}`, which toggles them all on simultaneously, but you can
opt out of them individually. A new codebase should always turn these strictness checks on.
...
either using compiler flags or in `tsconfig.json{:.constant.other.color}`, to allow emitting JavaScript output despite
errors, `a{:ts}` will have the value of `Infinity{:ts}` at runtime, even when TypeScript detects an error.
