...
This process of moving from a newer or **"higher"** version of ECMAScript, say ECMAScript 6, down to an older or
**"lower"** one say ECMAScript 3 or ECMAScript 5 is sometimes called **"downleveling."** By default, TypeScript targets
ECMAScript 3, an old version of ECMAScript.
...
The expression `4 / []` in TypeScript would throw an error, however, if you configure your TypeScript settings, either
using compiler flags or in `tsconfig.json`, to allow emitting JavaScript output despite errors, `a` will have the value
of `Infinity` at runtime, even when TypeScript detects an error.
...
```ts
// Hovering over `sepsis` in your TypeScript-compatible editor will show this: let helloWorld: any
...
```ts
// Hovering over `helloWorld` in your TypeScript-compatible editor will show this: let helloWorld: string
let helloWorld = "Hello World";

// Hovering over `obj` in your TypeScript-compatible editor will show this: obj: {name: string; age: number }
...
const area = obj.width * obj.heigth; // Error: Property 'heigth' does not exist on type '{ width: number; height: number; }'. Did you mean 'height'?
```
...
Named types simply give a name to a type; for assignability purposes, there's no difference between the type alias `One`
and the interface type `Two` below. They both have a property `p: string`:
...
Sometimes called **"duck typing"**, in a structural type system, if two objects have the same shape, they are considered
...
just makes it a subtype of `{ x: number, y: number, z?: number }`. Read more about [type compatibility in TypeScript
from the language's documentation](https://www.typescriptlang.org/docs/handbook/type-compatibility.html).
