...
DogHouse.saySomething(dogHouse.resident); // Error: Argument of type 'Animal' is not assignable to parameter of type 'Dog'...
```
...
Because `public` is already the default visibility modifier, you don't need to write it on a class member, but might
...
g.greet(); // Hello, hi
g.howdy(); // Howdy, hi
g.getName(); // Error: Property 'getName' is protected and only accessible withing class 'Greeter' and its subclasses.
```
...
console.log(b.x); // Error: Property 'x' is prviate and only accessible within the class 'Base'.
```
...
console.log(MyClass.x); // Error: Property 'x' is private and only accessible with class 'MyClass'
```
...
Note that `Derived` was already able to read and write `m`, so this doesn't meaningfully alter the "security" of this
situation. The main thing to note here is that in the derived class, we need to be careful to repeat the `protected`
modifier if this exposure isn't intentional.

Because `private` members aren't visible to derived classes, a derived class can't increase or decrease their
visibility:
...
class Derived extends Base {
  // Error: Class 'Derived' incorrectly extends base class 'Base'. Property 'm' is private in type 'Derived' but not in type 'Base'.
  private m = 15;
}

const d = new Derived();
console.log(d.m); // Error: Property 'm' is private and only accessible within class 'Derived'.
```
...
class Derived2 extends Base {
  ...
  f2(other: Derived1) {
    other.x = 10; // Erorr: Property 'x' is protected and only accessible within class 'Derived1' and its subclasses.
  }
}
```

This is because accessing `x` in `Derived2` should only be legal from `Derived2`’s subclasses, and `Derived1` isn't one
of them. Moreover, if accessing `x` through a `Derived1` reference is illegal (which it certainly should be!), then
accessing it through a base class reference should never improve the situation.
...
This means that JavaScript runtime constructs like `in` or simple property lookup can still access a `private` or
`protected` member:
...
`private` also allows access using bracket notation during type checking. This makes `private`-declared fields
...
Unlike TypeScripts's `private`, JavaScript’s private fields (`#`) remain private after compilation and do not provide
the previously mentioned escape hatches like bracket notation access, making them hard private.
...
garfield["meowAmount"]; // OK because "private" allows access with bracket notation as an escape hatch.
garfield.meowAmount; // Error: Property 'meowAmount' is private and only accessible within class 'Cat'.

const fido = new Dog();
fido["#barkAmount"]; // Error: Element implicitly has 'any' type because expresson of type '#barkAmout' can't be used to index type 'Dog'.
fido.#barkAmount; // Error: Property '#barkAmount' is not accessible outside class 'Dog' because it has a private identifier.
```
...
functions that can be invoked with `new`, certain `static` names can't be used. Function properties like `name`,
...
Remember that types are always fully erased! At runtime, there's only one `Box.defaultValue` property slot. This means
that setting `Box<string>.defaultValue` (if that were possible) would also change `Box<number>.defaultValue` - not good.

## In TypeScript, you can use `this is Type` in the return position for methods in classes and interfaces.

When mixed with a type narrowing (e.g. `if` statements) the type of the target object would be narrowed to the specified
`Type`. For example:
...
case removes an `undefined` from the value held inside `box` when `hasValue` has been verified to be true:
...
// Hovering over `box.value` in your TypeScript-compatible editor will show this: (property) Box<string>.value?: string
box.value;

if (box.hasValue()) {
  // Hovering over `box.value` in your TypeScript-compatible editor will show this: (property) value: string
  box.value;
}
```
...
modifiers `public`, `private`, `protected`, or `readonly`. The resulting field gets those modifier(s):
...
console.log(a.x); // 1
console.log(a.y); // Error: Property 'y' is protected and only accessible within class 'Params' and its subclasses.
...
We can't instantiate `Base` with `new` because it's abstract. Instead, we need to make a derived class and implement the
abstract members:
...
```ts
class Derived extends Base {
  // Erorr: Non-abstract class 'Derived' does not implement inherited abstract member getName from class 'Base'.
}
```
...
`greet`, it's perfectly legal to write this code, which would end up constructing an abstract class:
...
Now TypeScript correctly tells you about which class constructor functions can be invoked - `Derived` can because it's
concrete, but `Base` cannot.
...
const name = createCatName(); // Error: 'createCatName' cannot be used as a value because it was imported using 'import type'.
```

The second is an inline type import. TypeScript 4.5 also allows for individual imports to be prefixed with `type` to
indicate that the imported reference is a type:
...
features in `namespaces` exist in ES Modules and TypeScript recommends you use that to align with JavaScript’s
...
interface Box {
  width: string; // Error: Subsequent property declarations must have the same type. Property 'width' must be of type 'number', but here has type 'string'.
}
```
...
Of note, too, is that in the case of interface `A` merging with later interface `A`, the second interface will have a
higher precedence than the first.
