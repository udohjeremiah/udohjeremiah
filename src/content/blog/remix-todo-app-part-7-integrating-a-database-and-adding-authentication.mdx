...
promised to cover everything you'd use daily in Remix, and I believe it has been delivering on that promise 😉. In
...
tasks—if multiple users add tasks simultaneously, data overlaps, and everything is lost on restart. We'll fix this by
...
JWT—not the JWT itself—making JWTs **stateless**.
...
challenges—especially around token invalidation. In session-based authentication, a compromised session can be
...
Once this is done, you can sign up and refresh your MongoDB database—either through the MongoDB Compass GUI or directly
on mongodb.com—to view your new user document.
...
The final step is to handle the form submission in the route's `action`. Update the `/signin` route with the following
code changes:
...
The `action` retrieves the cookie from the `request`, and passes it to `getSession` which retrieves the session data.
...
Check the session data for the necessary information—if absent, redirect to `/signin`; otherwise, proceed with handling
the request.
...
accessing the `/` route—you should be redirected to `/signin` if not authenticated. To regain access, fill out the sign
...
With this functionality in place, users can delete their accounts if they choose to do so. Voilà!
...
dedicated, even during the long stretches. 😊 I hope this series met its goal of covering 80% of the key concepts you'll
