...
**not** force functions to not return something. Another way to say this is a contextual function type with a
...
needs something to be the other kind of declaration. If you would like a heuristic, use `interface` until you need
...
addOne(x); // Argument of type 'string' is not assignable to parameter of type 'number'.
```
...
specific possible type of a value at a given position. It looks at these special checks
...
because they usually create a new object. You can write a construct signature by adding the `new` keyword in front of a
call signature:
